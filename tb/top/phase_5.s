...
flush_test_beq:
    ...
    beq x9, x3, flush_beq_target       # x9=3, x3=3 so taken, 0 NOP after
    addi x23, x0, 99                    # wrong-path, should be flushed


flush_beq_target:

    beq x23, x0, flush_test_jal         # x23 should still be 0 if flush correctly
    addi x10, x0, 45
    NOP
    NOP
    beq x0, x0, halt
...
flush_test_jal:
    ...
    jal x14, jal_landing                 # x14 = addr of next instr
    addi x31, x0, 77                     # wrong-path candidate on 1st fetch; also deliberate 2nd-pass landing target


jal_landing:

    beq x15, x0, jal_first_pass          # x15 = phase flag, starts at 0

    # only reached via deliberate jalr return below (2nd pass)
    beq x0, x0, flush_test_not_taken     # got back safely -> jal's link value was correct, move on


jal_first_pass:

    beq x31, x0, jal_first_pass_ok       # x31 should still be 0 (wrong-path correctly flushed)
    addi x10, x0, 46
    NOP
    NOP
    beq x0, x0, halt
...
jal_first_pass_ok:
    ...
    addi x15, x0, 1                      # mark phase flag before returning
    jalr x0, 0(x14)                      # jump back to the addi x31, x0, 77
                            # if x14 (link) is correct, lands exactly there, x31=77, falls into jal_landing again with x15=1



flush_test_not_taken:

    beq x9, x4, flush_test_end           # x9=3, x4=4 NOT taken, should just fall through, 0 NOP
    addi x23, x0, 1                    #should execute



flush_test_end:

    beq x23, x1, initialize_loop               # if not-taken worked correctly, x23 should be 1
    addi x10, x0, 47
    NOP
    NOP
    beq x0, x0, halt
