...
async def phase_5(dut):
    ...
    loop_branch_pc = 0xB14
    exit_marker_pc = 0xB18
    loop_target_pc = 0xB10
    ...
    while cycles < 1000:
        ...
        try:
            ...
            if current_instr == 0xFE079EE3:
                dut._log.info(f'{current_pc}')
            '''

            if current_pc == loop_branch_pc:
                visit_cycles.append(cycles)

            if current_pc == loop_target_pc:
                target_cycles.append(cycles)
    ...
    dut._log.info("------------------------------------------------------------------------------------------")
    dut._log.info("|Error code: 45 = flush failed: taken beq did not squash wrong-path instruction          |")
    dut._log.info("------------------------------------------------------------------------------------------")
    dut._log.info("|Error code: 46 = flush failed: jal did not squash wrong-path instruction                |")
    dut._log.info("------------------------------------------------------------------------------------------")
    dut._log.info("|Error code: 47 = not-taken branch incorrectly flushed a legitimate fallthrough instr    |")
    dut._log.info("------------------------------------------------------------------------------------------")
    dut._log.info("|\033[1m\033[34m                            Epic Error Codes Meaning Table Above                        \033[0m\033[0m|")
    dut._log.info("------------------------------------------------------------------------------------------")

    dut._log.info("To view waveform use \033[34mgtkwave ../cocotb_sim_phase_5/soc_top.fst ../phase5.gtkw\033[0m")


    assert return_code == 1, f'\033[31m\033[1mTEST FAILED >:(\033[0m\033[0m: assembly code returned error code: \033[31m{return_code}\033[0m'
    ...
    for i in range(min(len(visit_cycles), len(target_cycles)-1)):
        deltas.append(target_cycles[i+1] - visit_cycles[i])

    exit_cost = exit_marker_cycle - visit_cycles[-1]
    ...
    dut._log.info(f"\033[33mDynamic branch prediction\033[0m success rate: \033[34m{success_rate:.2f}%\033[0m")
    ...
    dut._log.info("\033[32m\033[1mTEST SUCCESS :D\033[0m\033[0m Assembly program passed self checks and verified by python test function")
